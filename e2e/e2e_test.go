...
func Test_dispatch_call_on_tick(t *testing.T) {
	stdErr, kill := testutil.StartEnvoy(t, 8001)
	defer kill()
	var count int = 1
		...
		t.Run(tc.op, func(t *testing.T) {
			require.Eventually(t, func() bool {
				...
				return string(body) == tc.expBody &&
					testutil.CheckMessage(stdErr.String(), []string{
						`original request body: [original body]`},
						[]string{"failed to"},
					) && testutil.CheckMessage(string(body), []string{tc.expBody}, nil)
...
func Test_metrics(t *testing.T) {
	_, kill := testutil.StartEnvoy(t, 8001)
	defer kill()
	var count int
	require.Eventually(t, func() bool {
		res, err := http.Get("http://localhost:18000")
...
func Test_shared_data(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return testutil.CheckMessage(stdErr.String(), []string{fmt.Sprintf("shared value: %d", count)}, nil)
	}, 5*time.Second, time.Millisecond, stdErr.String())
}
