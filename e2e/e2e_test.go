...
func Test_network(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return checkMessage(stdErr.String(), []string{
			...
			"upstream cluster matadata location[region]=ap-northeast-1",
			"upstream cluster matadata location[cloud_provider]=aws",
			"upstream cluster matadata location[az]=ap-northeast-1a",
		})
	}, 10*time.Second, 100*time.Millisecond, stdErr.String())
}
