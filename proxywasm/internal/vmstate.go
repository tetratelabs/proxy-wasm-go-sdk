...
func RegisterRedisCallout(calloutID uint32, callback func(status, responseSize uint32)) {
...
func (s *state) registerRedisCallOut(calloutID uint32, callback func(status, responseSize uint32)) {
	r := s.pluginContexts[s.contextIDToRootID[s.activeContextID]]
